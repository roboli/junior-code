function obtenerNota() {
  return new Promise(function(resolve) {
    setTimeout(function () {
      const nota = Math.round(Math.random() * 10);
      resolve({ nota });
    }, 5000);
  });
}

function obtenerMensaje(resultado) {
  return new Promise(function(resolve, reject) {
    setTimeout(function () {
      const nota = resultado.nota;

      if (nota < 0 || nota > 10) {
        const error = new Error('Nota fuera de rango!');
        return reject(error);
      }

      let mensaje;

      if (nota >= 8) {
        mensaje = 'Excelente:';
      } else if (nota > 5) {
        mensaje = 'Bueno:';
      } else {
        mensaje = 'Decepcionante:';
      }

      resultado.mensaje = mensaje;

      resolve(resultado);
    }, 3000);
  });
}

function obtenerNumeroLetras(resultado) {
  return new Promise(function(resolve, reject) {
    setTimeout(function () {
      const nota = resultado.nota;

      if (nota < 0 || nota > 10) {
        const error = new Error('Nota fuera de rango!');
        return reject(error);
      }

      let numero;

      switch (nota) {
        case 0:
          numero = 'cero';
          break;
        case 1:
          numero = 'uno';
          break;
        case 2:
          numero = 'dos';
          break;
        case 3:
          numero = 'tres';
          break;
        case 4:
          numero = 'cuatro';
          break;
        case 5:
          numero = 'cinco';
          break;
        case 6:
          numero = 'seis';
          break;
        case 7:
          numero = 'siete';
          break;
        case 8:
          numero = 'ocho';
          break;
        case 9:
          numero = 'nueve';
          break;
        case 10:
          numero = 'diez';
          break;
      }

      resultado.numero = numero;

      resolve(resultado);
    }, 2000);
  });
}

function imprimirNota() {
  function mostrarError(error) {
    console.error('Ocurrio algo malo:', error);
  }

  function mostrarResultadoFinal(resultado) {
    console.log(resultado.mensaje, resultado.numero);
  }

  obtenerNota()
    .then(obtenerMensaje)
    .then(obtenerNumeroLetras, mostrarError)
    .then(mostrarResultadoFinal, mostrarError);
}
